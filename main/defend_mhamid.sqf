...
while {_run_2} do {
	if (task_4_1_qrf_spawned) then {
		initial_unitsNumber = { !(isPlayer _x) and { (side _x) == opfor } and { _x inArea _marker_defend_mhamid } } count allUnits;
...
while {_run_1} do {
	if ((task_4_1_qrf_spawned) || (task_4_1_skip)) then {
		curr_unitsNumber = { !(isPlayer _x) and { (side _x) == opfor } and { _x inArea _marker_defend_mhamid } } count allUnits;
